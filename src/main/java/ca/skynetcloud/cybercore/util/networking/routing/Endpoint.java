package ca.skynetcloud.cybercore.util.networking.routing;
...
public class Endpoint {
	public final BlockPos pos; // TEs can become invalidated or replaced, so get new ones when needed (not sure
	...
	public boolean equals(Object other) {
		...
		{
			return true;
		} else if (other instanceof Endpoint) { // if other object is an endpoint,
	 ...
	 * 
	 * Return FALSE if the handler cannot take the stack or if either the handler or
	 * the TE do not exist
	 * 
	 * This only simulates the insertion and does not affect the state of any
	 * itemstacks or inventories
	 * 
	...
	public boolean canInsertItem(World world, ItemStack stack) {
		...
		LazyOptional<IItemHandler> optionalHandler = te.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY,
				this.face);
		return optionalHandler.map(handler -> canInsertItem(handler, stack)).orElse(false);
	}

	// helper function used for the above method
	// the itemstack is the one passed into the above method, the item handler is
	// assumed to exist
	...
	// returns the slot index of the first available slot in the endpoint block's
	// inventory,
