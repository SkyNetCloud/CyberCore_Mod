...
public class BaseContainerCore extends AbstractContainerMenu {
	...
	public BaseContainerCore(int id, MenuType<?> powerFurncaeCon, Inventory inv,
			CoreEnergyTileEntity tileentity, int slots) {
		super(powerFurncaeCon, id);
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 9; x++) {
				addSlot(new Slot(inv, x + y * 9 + 9, 24 + x * 18, 107 + y * 18));
			}
		}

		for (int x = 0; x < 9; x++) {
			addSlot(new Slot(inv, x, 24 + x * 18, 165));
		}
