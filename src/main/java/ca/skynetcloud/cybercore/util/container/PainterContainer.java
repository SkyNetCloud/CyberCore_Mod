...
public class PainterContainer extends BaseContainerCore {
	public PainterContainer(int id, PlayerInventory inv) {
		this(id, inv, new CablePainterTE());
	}

	public PainterContainer(int id, PlayerInventory player, CablePainterTE tileentity) {
		...
		this.addSlot(new ChangeCheckSlot(tileentity, handler, 0, 34, 83, "slot.painter.input"));
		...
		this.addSlot(
				new SlotItemHandlerWithInfo(handler, tileentity.getEnergyOutSlot(), 167, 57, "slot.util.energyout"));
	}
...
class ChangeCheckSlot extends SlotItemHandlerWithInfo {
	private CablePainterTE te;

	public ChangeCheckSlot(CablePainterTE te, IItemHandler itemHandler, int index, int xPosition, int yPosition,
			String usage) {
