...
public class WorldHelperWrapper {
    public static List<ItemCableBlockEntity> getTubesAdjacentTo(Level world, BlockPos pos) {
        List<ItemCableBlockEntity> tes = new ArrayList<ItemCableBlockEntity>(6);
        for (Direction face : Direction.values()) {
            ...
            if (te instanceof ItemCableBlockEntity) {
                tes.add((ItemCableBlockEntity) te);
            }
        }
    ...
    public static Stream<ItemCableBlockEntity> getBlockPositionsAsTubeTileEntities(Level world,
        ...
        Stream<BlockEntity> filteredStream = teStream.filter(te -> te instanceof ItemCableBlockEntity);
        return filteredStream.map(te -> (ItemCableBlockEntity) te);
    }
    ...
    public static Direction getBlockFacingForPlacement(BlockPlaceContext context) {
        ...
        {
            isSprintKeyHeld = PlayerDataWrapper.getSprinting(context.getPlayer().getUUID());
        }
