...
public class CyberChunkGen extends ChunkGenerator<CyberChunkGen.Config> {
	protected final SharedSeedRandom randomSeed;
	private final INoiseGenerator surfaceDepthNoise;

	public CyberChunkGen(IWorld world, BiomeProvider provider) {
		super(world, provider, Config.createDefault());
		this.randomSeed = new SharedSeedRandom(this.seed);
		boolean usePerlin = true;
		this.surfaceDepthNoise = (INoiseGenerator) (usePerlin ? new PerlinNoiseGenerator(this.randomSeed, 3, 0)
				: new OctavesNoiseGenerator(this.randomSeed, 3, 0));
	}
	...
	public void func_225551_a_(WorldGenRegion region, IChunk chunk) {
		...
		for (int i1 = 0; i1 < 16; ++i1) {
			for (int j1 = 0; j1 < 16; ++j1) {
				...
				region.func_226691_t_(blockpos$mutable.setPos(k + i1, i2, l + j1)).buildSurface(sharedseedrandom, chunk,
						k1, l1, i2, d1, this.getSettings().getDefaultBlock(), this.getSettings().getDefaultFluid(),
			...
			if (l > 0) {
				for (int i1 = l; i1 >= l - 4; --i1) {
					if (i1 >= l - rand.nextInt(5)) {
						chunkIn.setBlockState(blockpos$mutable.setPos(blockpos.getX(), i1, blockpos.getZ()),
