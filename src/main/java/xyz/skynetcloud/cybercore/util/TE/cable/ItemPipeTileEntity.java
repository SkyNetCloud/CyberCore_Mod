...
public class ItemPipeTileEntity extends TileEntity implements ITickableTileEntity {
	...
	public ItemPipeTileEntity() {
		this(TileEntityNames.TE_TYPE_ITEM_PIPE);
	}
	...
	public boolean isItemPipeCompatible(ItemPipeTileEntity tube) {
		...
		if (thisBlock instanceof CyberCoreItemPipe && otherBlock instanceof CyberCoreItemPipe) {
			return ((CyberCoreItemPipe) thisBlock).isTubeCompatible((CyberCoreItemPipe) otherBlock);
		}
		return false;
	}
	...
	public List<Direction> getConnectedDirections() {
		...
		return CyberCoreItemPipe.getConnectedDirections(state);
	}
