...
import xyz.skynetcloud.cybercore.util.TE.TileEntityTypes;
...
public class CableTileEntity extends TileEntity implements ITickableTileEntity {
	...
	HashMap<BlockPos, Direction> producer = new HashMap<BlockPos, Direction>(),
			consumer = new HashMap<BlockPos, Direction>(), storages = new HashMap<BlockPos, Direction>();
	private boolean connectionUpdate = true;

	public CableTileEntity() {
		super(TileEntityTypes.POWER_BOX_TE);
	}
	...
	private void transferEnergy() {
		...
		if (connectionUpdate) {
			...
			for (PowerConnection con : this.connectionsMaster.get(0)) {
			...
			for (PowerConnection con : this.connectionsMaster.get(1)) {
				cap = getEnergyCap(con.getConnectedPos(), con.getFacing().getOpposite());
				if (cap != null) {
			...
			List<BlockPos> intersect_PC = producer.keySet().stream().filter(x -> consumer.containsKey(x))
