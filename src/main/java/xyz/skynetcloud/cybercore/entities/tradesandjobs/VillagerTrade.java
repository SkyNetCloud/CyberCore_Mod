...
public class VillagerTrade
{
	...
	private final int DollarsBuy, DollarsSell, neededTrustLevel; 
	
	public VillagerTrade(String name, List<ItemStack> inputs, List<ItemStack> outputs, int creditsBuy, int creditsSell, int neededTrustLevel )
	{
		this.name = name; 
		this.inputs = inputs; 
		this.outputs = outputs; 
		this.DollarsBuy = creditsBuy;
		this.DollarsSell = creditsSell; 
	...
	public CompoundNBT toNBT()
	{
		...
		nbt.putInt("Dollarsbuy", DollarsBuy);
		nbt.putInt("Dollarssell", DollarsSell);
		nbt.putInt("neededtrustlevel", neededTrustLevel);
		return nbt; 
	}
	
	public static VillagerTrade fromNBT(CompoundNBT nbt)
	{
		...
		return new VillagerTrade(nbt.getString("name"), inputs, outputs, nbt.getInt("Dollarsbuy") , nbt.getInt("Dollarssell") , nbt.getInt("neededtrustlevel"));
	}
	...
	public PacketBuffer toBuffer(PacketBuffer buf)
	{
		...
		buf.writeInt(DollarsBuy); 
		buf.writeInt(DollarsSell); 
		buf.writeInt(neededTrustLevel); 
		return buf; 
	}
	
	public static VillagerTrade fromBuffer(PacketBuffer buf)
	{
		List<ItemStack> inputs = new ArrayList<>(), outputs = new ArrayList<>(); 
		...
		return new VillagerTrade(name, inputs, outputs, buf.readInt(), buf.readInt(), buf.readInt());
	}
}
